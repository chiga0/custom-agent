...
### 5. Task Claim Mechanism

`custom-agent-docs/docs/zh/03-roadmap-status.md §5 ("当前活跃工作")` is the canonical task-claim ledger. Apply these rules to prevent two agents picking the same work:

- **Before pushing a branch**, the author agent MUST append a row to `§5` containing: `Work ID`, current state (`IN_PROGRESS`), agent identifier (e.g. `Claude Opus 4.7`, `GPT-5 Codex`), branch name placeholder, scope summary, conflict domain, and "next step".
- **Before claiming a Work ID**, an agent MUST read `§5` AND `§7`. If the target Work ID already has an `IN_PROGRESS` / `REVIEW` row, pick a different Work ID. Two agents picking the same row is a process violation and the later commit must be reverted.
- **Conflict-domain claim is stronger than Work-ID claim.** Even if Work IDs differ, two open PRs touching the same conflict domain listed in `03-roadmap-status.md §11` cannot proceed in parallel (the second waits). The active-work table makes this visible.
- **State transitions on the table are mandatory**: row added on branch push (`IN_PROGRESS`) → updated on PR open (`REVIEW`) → row removed and `§7` flipped to `DONE` on merge. Each transition is part of the PR diff or a follow-up direct-push docs commit.

### 6. Reviewer Routing

The first reviewer for any open PR is any non-author agent that is **not** currently working in the same conflict domain. Apply in order:

1. **Author cannot review their own PR.** GitHub structurally blocks self-approval; the rule extends to mainline-guardian reviews too — an author posting a "Round 2 PASS" on their own PR does NOT count as a review.
2. **Cross-conflict-domain bias is preferred.** A reviewer who is also active in the same conflict domain (e.g. another storage agent reviewing a storage PR) MAY review but should disclose the overlap in the review body.
3. **No agent may review more than two open PRs at the same time** to avoid serial bottlenecks; if all available reviewers are saturated, the PR queues until one frees up.
4. **A second reviewer is required when** the PR modifies `packages/schema`, an ADR, `rules/mainline.md`, `AGENTS.md`, or the CI workflow. For routine package-internal changes, one reviewer suffices.
5. **Every review comment ends with `Generated by <model>`** (Section 2). Authors and reviewers can both be identified from the PR thread.

### 7. Merge Authority

Currently the project has a single maintainer; merge authority is constrained by GitHub branch protection. Apply this discipline:

- A PR is mergeable only when the reviewer agent has posted a `mainline-guardian` Round 2 (or later) **PASS** comment after the final commit. Self-PASS comments by the author do not satisfy this rule.
- Because GitHub blocks self-approval, the maintainer merges with `gh pr merge --admin --merge` **only after** the reviewer agent's PASS is recorded. The merge commit and the reviewer's PASS comment together form the audit trail.
- Once a second non-author maintainer / agent identity exists (e.g. a CI bot account or a separate reviewer account), branch protection should be tightened to **require an `APPROVE` review by a non-author** and the `--admin` workaround retired.
- **No agent merges its own PR.** When the author is the only available human, the maintainer merges after the reviewer agent's PASS — the human action is "merge button" not "approve button."
- Force-merges that skip the PASS rule require a post-hoc `INCIDENT` entry in `03-roadmap-status.md §12` (decision log) explaining the override.
